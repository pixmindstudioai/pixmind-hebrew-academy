...
struct WebAppView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        // Tell the web it is running inside the native app, so it hides all purchasing UI.
        // Purchases happen only on the website (SUMIT); the app is for viewing content.
        let nativeFlag = WKUserScript(
            source: "window.__PIXMIND_NATIVE__ = 'ios';",
            injectionTime: .atDocumentStart,
            forMainFrameOnly: true
        )
        configuration.userContentController.addUserScript(nativeFlag)
